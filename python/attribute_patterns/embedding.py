...
def get_edge_list(graph, node_list, node_to_ix):  
    ...
    return [[node_to_ix[s], node_to_ix[t], w] for s, t, w in graph.edges(data='weight') if s in node_list and t in node_list]  
...
def calculate_centroids(period_embeddings, node_list, node_to_ix, time_to_graph_keys):  
    ...
    for node in node_list:
        node_to_centroid[node] = np.mean([period_embeddings[period][node_to_ix[node]] for period in time_to_graph_keys], axis=0).tolist()
...
def generate_embedding(df, time_to_graph):  
    ...
    for period, graph in time_to_graph.items():  
        period_embeddings[period] = generate_embeddings_for_period(graph, node_list, node_to_ix, node_to_label)  
