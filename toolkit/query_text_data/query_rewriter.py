...
async def rewrite_query(ai_configuration, query, concept_graph, top_concepts):
    ...
    """
    concepts = sorted(concept_graph.degree(), key=lambda x: x[1], reverse=True)[:top_concepts]
    concepts_str = ", ".join([concept for concept, _ in concepts])
    messages = utils.prepare_messages(prompts.query_anchoring_prompt, {'query': query, 'concepts': concepts_str})
