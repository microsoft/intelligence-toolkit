...
async def answer_question(
    ...
    clustered_texts = [[f"{cid}: {processed_chunks.cid_to_text[cid]}" for cid in cids] for cids in clustered_cids.values()]
                ...
                claim_summaries.append(
                    {
                        ...
                        'sources': {
                            source: text for source, text in processed_chunks.cid_to_text.items() if source in supporting_sources.union(contradicting_sources)
                        }
                    }
                )
