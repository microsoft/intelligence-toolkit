...
The `Detect Case Patterns` workflow generates intelligence reports on patterns of attribute values detected in streams of case records.
...
`Detect Case Patterns` can be used to detect patterns of attributes in any timestamped set of case records, including mock records created using `Generate Record Data` and synthetic case records anonymized using `Anonymize Case Data`.
