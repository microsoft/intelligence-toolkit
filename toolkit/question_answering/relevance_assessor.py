...
def assess_relevance(
    ...
    batched_chunks = [
        search_chunks[i : i + relevance_test_batch_size]
        for i in range(0, len(search_chunks), relevance_test_batch_size)
    ]
    batched_messages = [
        [
            utils.prepare_messages(
                prompts.chunk_relevance_prompt, {"chunk": chunk, "question": question}
            )
            for chunk in batch
        ]
    ...
    for mx, mapped_messages in enumerate(batched_messages):
        ...
        if len(test_history) + len(mapped_messages) > relevance_test_limit:
            mapped_messages = mapped_messages[
                : relevance_test_limit - len(test_history)
            ]
        mapped_responses = asyncio.run(
            helper_functions.map_generate_text(
