...
def _generate_embeddings_for_period(graph, node_list, node_to_label, correlation, diaga, laplacian, max_level):
    ...
    for level in range(0, max_level+1):
        labels = np.array([node_to_label[node][level] if node in node_to_label else -1 for node in node_list ]).reshape(
            (
                num_nodes,
                1,
            )
    ...
    for level in range(0, max_level+1):
      ...
      if level_embeddings[level].shape[1] == embedding_length:
        ...
        normalized_vectors[level] = normalize(tsvd.transform(level_embeddings[level].toarray()))
    ...
    for node in node_list: 
      for level in range(0, max_level+1):
        ...
        if level == 0:
            ...
            concat_vectors[level][node] = normalized_vectors[level][node_to_ix[node]]
        else:
...
def generate_graph_fusion_encoder_embedding(period_to_graph, node_to_label, correlation, diaga, laplacian, max_level, callbacks=[]):
