...
import intelligence_toolkit.detect_case_patterns.model as model
...
class DetectCasePatterns(IntelligenceWorkflow):
    ...
    def _prepare_graph(
        ...
        self.detect_patterns_df, self.period_to_graph = model.prepare_graph(
            self.dynamic_graph_df, min_edge_weight, missing_edge_prop
        )

    def generate_embedding_model(self):
        node_to_label_str = dict(
            self.dynamic_graph_df[["Full Attribute", "Attribute Type"]].values
        )
    ...
    def detect_patterns(self, min_pattern_count, max_pattern_length):
        self.min_pattern_count = min_pattern_count
        self.max_pattern_length = max_pattern_length
        (self.patterns_df, self.close_pairs, self.all_pairs) = model.detect_patterns(
    ...
    def explain_pattern(
        ...
        if attribute_counts is None:
            attribute_counts = self.compute_attribute_counts(
                selected_pattern, selected_pattern_period
            )
        variables = {
            "pattern": selected_pattern,
            "period": selected_pattern_period,
            "time_series": self.time_series_df[
                self.time_series_df["pattern"] == selected_pattern
            ].to_csv(index=False),
