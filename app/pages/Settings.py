...
def main():
    ...
    with st.spinner("Fetching models..."):
        try:
            ...
            st.error("Invalid key. Please check your OpenAI key.")
            return
    gpt_list = [x.id for x in openai_models if 'gpt' in x.id]
    
    embeddings_list = [x.id for x in openai_models if 'embedding' in x.id]
    model_change = st.selectbox("OpenAI GPT model", gpt_list, index=gpt_list.index(sv.generation_model.value))
    ...
    embedding_model_change = st.selectbox("OpenAI embedding model", embeddings_list, index=embeddings_list.index(sv.embedding_model.value))
