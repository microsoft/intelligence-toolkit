...
def create():
    ...
    uploader_tab, detect_tab, explain_tab = st.tabs(['Create graph model', 'Detect patterns', 'Explain patterns'])
    df = None
    with uploader_tab:
        ...
        with model_col:
            ...
            att_cols = [col for col in sv.attribute_binned_df.value.columns.values if col not in ['Subject ID', time_col] and st.session_state[f'{workflow}_{col}'] == True]
    ...
    with detect_tab:
        if not ready or len(sv.attribute_final_df.value) == 0:
            ...
            with c2:
                ...
                functions.compute_attribute_counts(sv.attribute_final_df.value, selected_pattern, time_col, selected_pattern_period)

                if selected_pattern != '' and False:
                ...
                with ai_placeholder:
                    variables = {
                        ...
                        'time_series': tdf.to_csv(index=False),
                        # 'attribute_counts': att_counts
                    }
