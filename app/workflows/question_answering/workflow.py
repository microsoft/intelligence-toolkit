    ...
    with search_tab:
        ...
        if sv.last_question.value != "" and regenerate:
            ...
            def on_progress(message):
                progress_placeholder.dataframe(pd.DataFrame(columns=["Relevant text chunks"], data=message), hide_index=True, height=700, use_container_width=True)
            def on_answer(message):
                answer_placeholder.dataframe(pd.DataFrame(columns=["Partial answers"], data=message), hide_index=True, height=700, use_container_width=True)

            answers, chunks = search_answers.search_answers(
                question,
                sv.text_to_chunks.value,
                sv.chunk_to_concepts.value,
                sv.concept_to_chunks.value,
                ...
                embedding_cache=sv_home.save_cache.value,
                select_logit_bias=5,
                chunk_successive_no_break=10,
                community_successive_no_break=5,
                community_chunk_successive_no_break=5,
