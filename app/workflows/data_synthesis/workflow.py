...
def create():
    ...
    with prepare_tab:
        ...
        with uploader_col:
            util.ui_components.single_csv_uploader(sv.synthesis_last_file_name, sv.synthesis_raw_sensitive_df, sv.synthesis_binned_df)
        with model_col:
            with st.expander('Set subject identifier', expanded=True):
                ...
                if identifier == 'ID column':
                    sv.synthesis_subject_identifier.value = st.selectbox('Select subject identifier column', options=sv.synthesis_raw_sensitive_df.value.columns)
                    sv.synthesis_raw_sensitive_df.value = sv.synthesis_raw_sensitive_df.value.melt(id_vars=[sv.synthesis_subject_identifier.value], var_name='Attribute', value_name='Value')
                    # convert back to wide format
                    sv.synthesis_raw_sensitive_df.value = sv.synthesis_raw_sensitive_df.value.pivot(index=sv.synthesis_subject_identifier.value, columns='Attribute', values='Value')
                else:
                    sv.synthesis_raw_sensitive_df.value['Subject ID'] = [i for i in range(len(sv.synthesis_binned_df.value))]
            ...
            with st.expander('Quantize numeric attributes', expanded=False):
                ...
                for col in sv.synthesis_process_columns.value:
                    if sv.synthesis_raw_sensitive_df.value[col].dtype in ['float64', 'int64']:
                        binnable_cols.append(col)

                selected_binnable_cols = st.multiselect('Select numeric attributes to quantize', binnable_cols, help='Select the columns you want to quantize. Quantizing numeric columns into bins makes it easier to synthesize data, but reduces the amount of information in the data. If you do not select any columns, no binning will be performed.')
                ...
                if st.button('Quantize selected columns'):
                    if num_bins == 0:
            ...
            median_counts = []
            distinct_counts = []
            num_records = len(sv.synthesis_raw_sensitive_df.value)
                ...
                with b2:
                    if st.button('Synthesize data'):
                        with st.spinner('Synthesizing data...'):
                            sv.synthesis_sensitive_df.value = sv.synthesis_binned_df.value[sv.synthesis_process_columns.value].copy(deep=True)
    ...
    with queries_tab:
        if len(sv.synthesis_synthetic_df.value) == 0:
            st.markdown('Please upload and synthesize data before performing queries.')
        else:
