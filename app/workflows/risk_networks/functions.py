...
def get_entity_graph(G, selected, links_df, width, height, attribute_types):
    ...
    if len(all_nodes) > 0:
        max_risk = max([G.nodes[node]['flags'] for node in all_nodes])
...
def simplify_graph(C):
    ...
    S = merge_nodes(S, lambda x, y : # merge if overlapping types or values
...
def project_entity_graph(sv):
    ...
    trimmed_nodeset = set([t[0] for t in trim]).union(sv.network_additional_trimmed_attributes.value)
...
def build_integrated_flags(sv):
    ...
    overall_df = sv.network_integrated_flags.value[['qualified_entity', 'count']].groupby('qualified_entity').sum().reset_index()
    sv.network_max_entity_flags.value = overall_df['count'].max()
    sv.network_mean_flagged_flags.value = round(overall_df[overall_df['count'] > 0]['count'].mean(), 2)
...
def build_network_from_entities(sv, G, nodes):
        ...
        for ent_neighbor in ent_neighbors:
            if ent_neighbor not in trimmed_nodeset:
                if ent_neighbor.startswith(config.entity_label):
                    ...
                    N.add_node(ent_neighbor, type=ent_neighbor.split(config.att_val_sep)[0], flags = 0)
                    N.add_edge(node, ent_neighbor)
                    ...
                    for att_neighbor in att_neighbors:
                        if att_neighbor not in trimmed_nodeset:
                            if not att_neighbor.startswith(config.entity_label):
                                N.add_node(att_neighbor, type=att_neighbor.split(config.att_val_sep)[0], flags = 0)
                                fuzzy_att_neighbors = set(G.neighbors(att_neighbor)).union(sv.network_inferred_links.value[att_neighbor])
                                for fuzzy_att_neighbor in fuzzy_att_neighbors:
                                    if fuzzy_att_neighbor not in trimmed_nodeset:
                                        N.add_node(fuzzy_att_neighbor, type=fuzzy_att_neighbor.split(config.att_val_sep)[0], flags = 0)
    ...
    if len(sv.network_integrated_flags.value) > 0:
        ...
        for node in flagged_nodes:
            if node in N.nodes():
                N.nodes[node]['flags'] = fdf[fdf['qualified_entity'] == node]['count'].sum()
    return N
